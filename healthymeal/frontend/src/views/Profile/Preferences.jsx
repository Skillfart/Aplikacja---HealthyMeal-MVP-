...
export const Preferences = () => {
  ...
  const handleExcludedProductAdd = () => {
  ...
  const handleExcludedProductRemove = (index) => {
    setPreferences(prev => {
      const excludedProducts = Array.isArray(prev.excludedProducts) ? [...prev.excludedProducts] : [];
      return {
        ...prev,
        excludedProducts: excludedProducts.filter((_, i) => i !== index)
      };
    });
  };
      ...
      <form onSubmit={handleSave}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.formGroup}>
            <div className={styles.inputGroup}>
              <input
                ...
                placeholder="Dodaj produkt do wykluczenia"
                className={styles.input}
              />
              <button
                type="button"
                onClick={handleExcludedProductAdd}
          ...
          {Array.isArray(preferences.excludedProducts) && preferences.excludedProducts.length > 0 && (
            <div className={styles.excludedProducts}>
              {preferences.excludedProducts.map((product, index) => (
                <div key={index} className={styles.excludedProduct}>
                  <span>{product}</span>
                  <button
                    type="button"
                    onClick={() => handleExcludedProductRemove(index)}
